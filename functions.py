...
"""
    In the new versions of the functions, we have simple removed the inclusion of Winter Wheat, since we  are no longer using it for the latest versions for our Project.
...
"""
    We are going to add SHAP values to this function so that we can see what the most influential features were for the model
"""
def train_xgb_new(params, full_df, num_features):
    # Setting up our plotting config
    fig, axes  = plt.subplots(3,2, figsize=(15, 15))
    axes = axes.flatten()
    ...
    for i, _ in enumerate(years):
        ...
        # Instanitaiting our Model
        xgb_model = xgboost.XGBClassifier(**params, early_stopping_rounds=50, importance_type='gain')
        ...
        # Returning only the top specified num of features
        ...
        feature_importances[year_test] = xgb_importances

        # Now we are going to add our SHAP code
        explainer = shap.TreeExplainer(xgb_model)
        shap_values = explainer(X_test)

        # Appending it to our Shap_results Dict
        shap_results[year_test] = shap_values
        ...
        # Visualizing our Confusion Matrix
        disp = ConfusionMatrixDisplay(confusion_matrix=matrix, 
                                      display_labels=['Barley', 'Oats', 'Spring_Wheat'])
        ax.set_title(f'Conf Matrix for {year_test}. Accuracy: {accuracy_test}')
        disp.plot(ax=ax)
    ...
    print(f'MEAN ACCURACY: {np.mean(accuracies)}')
    print(f'MEAN TRAIN ACCURACY: {np.mean(train_accuracies)}')
    return feature_importances, shap_results
...
"""
Aslo adding SHAP values
"""

def train_model_new(model, full_df):
    # Setting up our plotting config
    fig, axes  = plt.subplots(3,2, figsize=(15, 15))
    axes = axes.flatten()
